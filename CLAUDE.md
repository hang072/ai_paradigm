...
- **P91 PDF 解析统一走 PyMuPDF sidecar**: 替代 P85 的 pdfcpu + Qwen-VL 双路径。 `backend/sidecars/pymupdf-extract/` 起 FastAPI, `POST /extract` 接 `{blob_b64}` 返 `{markdown, raw_json, pages_n, images[], tables_count, elapsed_ms}`; `GET /health` 健康检查。 Go 客户端 `internal/embedding/pymupdf.go` 走 `net/http` + 1 次 transient 重试(照抄 vision.go 模式), 启动时 ping `/health`, 失败 → fatal(全替决策不静默降级)。 parser 切到 `internal/parser/pymupdf_parser.go`, `main.go` 调 `parser.SetPymupdfClient` 注入。 落盘走 `embedding.PersistImages(kbID, docID, []ImageBlob, maxBytes)`, JSON 形状跟 P85 `ExtractedImage` 完全一致, 前端 `DocStructureImage` / `imagesJSONContains` 零改动兼容。 抽图率 100% (41/41 PoC), 1-2.5s/PDF(vs VLM 30-60s)。 docker-compose 加 `pymupdf-sidecar` 服务(本地 build + 8002), 加可选 `backend` service(`--profile backend up`); 镜像 `backend/Dockerfile` multi-stage golang → alpine 非 root。 **已知限制**:扫描件无文本层返近空 markdown, **不 fallback VLM**; sidecar 不可用时 `go run ./cmd/server` 启动失败, 需先启 sidecar。 **P91 验证样本** (5 页医学 PDF "发发发" KB `kb-2f695723` / doc `doc-55bec705`): pymupdf 抽 2 张 PNG — image_001=11.4KB page 2 / image_002=18.9KB page 4, 磁盘 + sidecar `images_json` 元数据一致, `GET /api/kb/.../images/...` 200 + `image/png`, 抽不到的 page 返 404 (符合预期: vector PDF + 文本层无 embedded bitmap, PyMuPDF 也抽不出)。 **vite proxy 配套**: `frontend/vite.config.ts` 加 `proxy: { '/api': 'http://127.0.0.1:8001' }`, 浏览器地址栏直输 `localhost:5173/api/kb/.../images/...` 走 vite proxy 拿图(axios 走 `VITE_API_BASE` 不受此影响); `frontend/src/router/index.tsx` 加 `path:'*'` 兜底, 替换 React Router 默认 "💿 Hey developer" 为 AntD `Result status=404` 友好页。 **测试**: 12 个 unit (`TestPersistImages_*` 6 + `TestKbImages_*` 6) + 1 个 e2e (`TestPymupdfE2E`), 全绿, 11.9s。
- **P92 5 个小 gap 一锅端**: 一并清掉 `TODO.md` 列的 5 个已知小 gap + 1 个顺带发现真 bug:
  1. **Gap 1** 「存 localStorage」文案错位 → 改为 "持久化到后端 (SQLite settings 表, key=llm_prefs)"
  2. **Gap 2** 「测试连接」假成功 → 后端新 `POST /api/settings/llm/test`, 真发最小 chat (eino-ext Generate 透 401/超时), 8s ctx timeout 防卡 UI; `Provider` interface 加 `Probe(ctx) error` 方法, `OpenaiProvider` 用 Generate 调 system "ping" 实现, `Unavailable{}.Probe` 返 `ErrUnavailable`
  3. **Gap 3** 「今日待办」/tasks/:id 跳转 → 当前工作树已修, 不动
  4. **Gap 4** 模板卡片缺「复制」→ 后端新 `POST /api/templates/{id}/fork` (深拷 nodes/edges/tags/parameter_schema, builtin→false, 自动写 v1); 卡片 actions 加 `<CopyOutlined>` 走 `TemplatesApi.fork(t.id)`。**顺带修真 bug**: 编辑 builtin 模板的 performSave 原本走 PUT(本地 fork 的新 id 后端没记录, 返 404), 现改成 `if (srcBuiltinId) { fork → update 新 id → 清 srcBuiltinId }` 三步, 后续 save 就是普通 PUT
  5. **Gap 5** mock `/api/chat/reply` 直连 LLM → 改成优先 `fetch(${VITE_API_BASE}/api/chat/reply)` 走真后端, 失败回落 `callDirectChatReply` → mock。 `testLlmConnection` 同样走 fetch (避开 axios client 命中 mock adapter 递归)
  - **抽出的 `ResolveBaseURL(provider, override)` 公开 helper**: 7 个内置 vendor (deepseek/claude/openai/tongyi/zhipu/baidu/dianciyuann) 的 baseURL 推断逻辑从 `ConfigManager.Update` 抽出, 让 `/llm/test` 端点复用 (避免 copy-paste)
  - **测试**: 后端 `TestResolveBaseURL` (10 case) + `TestUnavailable_Probe` (no network) 全绿; 3 个 engine test 的 mock Provider 补 `Probe` 实现以满足新 interface 方法; `go test ./...` 全绿, E2E 验证 fork / /llm/test 4 个 case 全过
